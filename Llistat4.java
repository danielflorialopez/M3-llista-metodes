...
public class Llistat4 {
	//SI NO ES FA UN SCANNER GLOBAL HI HAURAN WARNINGS PER NO PODER TANCAR ELS SCANNERS (?)
	//SENSE NI UN SOL BREAK HO FARÀ RITA LA CANTAORA
	/**
	 * Es fa una instància de la classe Llistat4 (?)
	 * Se separa el programa principal a un altre mètode anomenat inici()
	 */
	 ...
	 * No té return
	 * 
	 * Es declara l'array alumnes d'una mida de 25
	 * Es declaren les altres variables:
	 * element fa referència al nom o cognom de l'alumne
	 * opcio fa referència a triar entre "primer" i "darrer"
	 * posicio fa referència a la posició de l'array
	 * operacio fa referència al número d'opció del menú
	 * num_posicions fa referència a la distància entre dos elements de dins de l'array
	 * 
	 * S'inicialitza un bucle per a que segueixi iterant mentres el número d'operacio no correspongui a la de sortir
	 * S'usa un switch per a passar el número d'operacio i triar una opció del menú
	 * Es comprova  amb altres mètodes que l'input sigui del tipus desitjat i dins dels paràmetres que es necessiten
	 */
	public void inici() {
	...
	public void inserir(String[] array, String elem, int pos) {
		...
		for (int i = 0; i < array.length; i++) {
			...
			if (array[i] != null) System.out.println((i + 1) + ". " + array[i]);
		}
	}
	
	public void localitzar(String[] array, String elem) {
		
		if (Arrays.asList(array).indexOf(elem) == -1) System.out.println("No s'ha trobat cap alumne amb aquest nom.");	
		else System.out.println("L'alumne està a la posició " + (Arrays.asList(array).indexOf(elem) + 1) + "\n");
	}
	...
	public void suprimirDada(String[] array, String elem, int num_pos) {
		...
		for (int i = 0; i < array.length; i++) { //es buiden els elements coincidents
			...
			if (array[i] != null && array[i].equals(elem)) array[i] = null;
		} 
		...
		for (int i = 0; i < array.length; i++) {//es compten les posicions fins el següent not null
			...
			if (array[i] == null) {
				...
				for (int j = i; j < array.length; j++) {//s'arrosseguen els valors tantes posicions com s'haiguen trobat
					
					if (j >= array.length - num_pos) array[j] = null;
					else array[j] = array[j + num_pos];
				}
			}
		}
	}
	...
	public void primerDarrer(String[] array, String opc) {
		...
		// busca primer element de l'array
			case "primer":
				...
				if (array[0] != null)	System.out.println(array[0]);
				break;
	...
	public void localitzarEnOrdenada(String[] array, String elem) {
		...
		System.out.println("L'element inserit està a la posició " + Arrays.asList(array).indexOf(elem));
	...
	public String comprovarOpcio() {
		...
		while (!opc.matches("primer") || !opc.matches("darrer")) {
